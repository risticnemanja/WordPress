<?php
	...
	if ( ! function_exists( "APBD_GetKarnelClass" ) ) {
		function APBD_GetKarnelClass() {
			if(class_exists("AppsBDKarnel")){
				return "AppsBDKarnel";
			}elseif(class_exists("AppsBDKarnelPro")){
				return "AppsBDKarnelPro";
			}elseif(class_exists("AppsBDKarnelLiteV1")){
				return "AppsBDKarnelLiteV1";
			}else{
				return "AppsBDKarnelLite";
			}
		}
	}
...
';
	...
	{
		APBD_GetKarnelClass()::AppsbdLoader($session_id)   ;
	}
}
	...
	if ( ! function_exists("APBD_GetHTMLRadioBoxByArray")) {
		...
		function APBD_GetHTMLRadioBoxByArray($title,$name, $id, $isRequired, $options, $checkedValue, $isDisabled=false, $bgcolor = '#ffffff',$class="",$attr=array(),$disabled_options=[],$option_group_class='',$option_item_class=''){
			?>
            <div class="apbd-app-box-radio <?php echo $option_group_class; ?>">
				<?php
					foreach ($options as $key=>$value){
						$attrStr=" ";
						if(is_array($attr) && count($attr)>0){
                        ...
                        <label class="apbd-app-box-option <?php echo $option_item_class; ?>"> <input class="apbd-app-box-option-input <?php echo $class;?>" <?php echo $attrStr;?>
    ...
    function APBD_GetHTMLRadioBoxByArrayWithCols($option_group_class='',$option_item_class='',$title,$name, $id, $isRequired, $options, $checkedValue, $isDisabled=false, $bgcolor = '#ffffff',$class="",$attr=array(),$disabled_options=[]){
        ?>
        <div class="apbd-app-box-radio <?php echo $option_group_class; ?>">
            <?php
            foreach ($options as $key=>$value){
